...
def _test_find_all_motifs_full_matrix_iteration(data, m, lb_list_size):
    dist_gen = ZNormEuclidean(0.)
    dist_gen.prepare(data, data, m)
...
def test_find_all_motifs_full_matrix_iteration_normal_data():
    # Random data, 20 points
    data = np.array(
        [-1.61, -0.43, -0.43, 0.82, 0.42, 1.58, -0.46, 1.41, 1.31,
         -0.13, -0.05, 0.59, 1.76, -0.43, -0.14, -0.14, 1.07, 1.1, 0.84, -1.49])

    _test_find_all_motifs_full_matrix_iteration(data, 4, 1)
    _test_find_all_motifs_full_matrix_iteration(data, 4, 5)

    # Due to the large subseq length, some lower bound arrays will be empty
    _test_find_all_motifs_full_matrix_iteration(data, 10, 5)
...
@unittest.skip("Flat signals have undefined lower bounds.")
def test_find_all_motifs_full_matrix_iteration_data_with_flats():
